...
func MapRoom(raw RoomXML) (postgres.Room, error) {
	updateDate, err := parseDate(raw.UpdateDate)
	if err != nil {
		return postgres.Room{}, fmt.Errorf("parse UPDATEDATE for id=%d: %w", raw.ID, err)
	}

	startDate, err := parseDate(raw.StartDate)
	if err != nil {
		return postgres.Room{}, fmt.Errorf("parse STARTDATE for id=%d: %w", raw.ID, err)
	}

	endDate, err := parseDate(raw.EndDate)
	if err != nil {
		return postgres.Room{}, fmt.Errorf("parse ENDDATE for id=%d: %w", raw.ID, err)
	}

	roomType, err := parseInt32(raw.RoomType)
	if err != nil {
		return postgres.Room{}, fmt.Errorf("parse ROOMTYPE for id=%d: %w", raw.ID, err)
	}

	operTypeID, err := parseInt32(raw.OperTypeID)
	if err != nil {
		return postgres.Room{}, fmt.Errorf("parse OPERTYPEID for id=%d: %w", raw.ID, err)
	}

	prevID, err := parseNullableInt64(raw.PrevID)
	if err != nil {
		return postgres.Room{}, fmt.Errorf("parse PREVID for id=%d: %w", raw.ID, err)
	}

	nextID, err := parseNullableInt64(raw.NextID)
	if err != nil {
		return postgres.Room{}, fmt.Errorf("parse NEXTID for id=%d: %w", raw.ID, err)
	}
