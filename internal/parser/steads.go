...
type SteadXML struct {
	XMLName    xml.Name `xml:"STEAD"`
...
func MapStead(raw SteadXML) (postgres.Stead, error) {
	updateDate, err := parseDate(raw.UpdateDate)
	if err != nil {
		return postgres.Stead{}, fmt.Errorf("parse UPDATEDATE for id=%d: %w", raw.ID, err)
	}

	startDate, err := parseDate(raw.StartDate)
	if err != nil {
		return postgres.Stead{}, fmt.Errorf("parse STARTDATE for id=%d: %w", raw.ID, err)
	}

	endDate, err := parseDate(raw.EndDate)
	if err != nil {
		return postgres.Stead{}, fmt.Errorf("parse ENDDATE for id=%d: %w", raw.ID, err)
	}

	operTypeID, err := parseInt32(raw.OperTypeID)
	if err != nil {
		return postgres.Stead{}, fmt.Errorf("parse OPERTYPEID for id=%d: %w", raw.ID, err)
	}

	prevID, err := parseNullableInt64(raw.PrevID)
	if err != nil {
		return postgres.Stead{}, fmt.Errorf("parse PREVID for id=%d: %w", raw.ID, err)
	}

	nextID, err := parseNullableInt64(raw.NextID)
	if err != nil {
		return postgres.Stead{}, fmt.Errorf("parse NEXTID for id=%d: %w", raw.ID, err)
	}

	return postgres.Stead{
		ID:         raw.ID,
